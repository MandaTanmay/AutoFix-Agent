...
import { useSSERepair, type AgentPhase, type TimelineEvent } from '@/lib/useSSERepair'
...
function AgentFlow({ state, attempt, running, maxAttempts }: { state: AgentState; attempt: number; running: boolean; maxAttempts: number }) {
  const currentIndex = state === 'idle' ? -1 : (state === 'retrying' || state === 'failed') ? 0 : stages.findIndex((stage) => stage.key === state)
...
export default function AutoFixDashboard() {
  ...
  const { run, abort, events, phase, attempt, diagnosis, validation, running, error } = useSSERepair()
    ...
    <div className="min-h-screen bg-[#f7f8fa] text-[#202938]">
      ...
      <main className="mx-auto max-w-[1440px] px-5 py-8 lg:px-8">
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(320px,1fr)_minmax(380px,1.08fr)]">
          ...
          />
          <AgentFlow state={state} attempt={attempt || 1} running={running} maxAttempts={maxAttempts} />
        </div>
